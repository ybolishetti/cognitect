"""Tests for IRC 2021 §R310.1 — bedroom egress window/door
(engine/verifiers/rules/irc_r310_1.py)."""

from __future__ import annotations

from engine.layout import Opening, Wall
from engine.verifiers import CodeCheckContext
from engine.verifiers.rules.irc_r310_1 import IRC_R310_1_BedroomEgressWindow
from tests.test_layer_c import make_layout, make_rect_walls, make_room

RULE = IRC_R310_1_BedroomEgressWindow()
CTX = CodeCheckContext()


def test_bedroom_with_exterior_window_passes():
    walls = make_rect_walls("room_1", "r1")
    room = make_room("room_1", "bedroom", [w.id for w in walls])
    window = Opening(id="opening_win1", opening_type="window", wall_id="wall_r1_n", offset_ft=2.0, width_ft=3.0)
    layout = make_layout([room], walls, [window])
    assert RULE.check(layout, CTX) == []


def test_bedroom_with_exterior_door_passes():
    walls = make_rect_walls("room_1", "r1")
    room = make_room("room_1", "bedroom", [w.id for w in walls])
    door = Opening(id="opening_door1", opening_type="door", wall_id="wall_r1_s", offset_ft=2.0, width_ft=3.0)
    layout = make_layout([room], walls, [door])
    assert RULE.check(layout, CTX) == []


def test_bedroom_with_only_interior_door_fails():
    # room_1: 3 exterior walls + 1 interior wall (shared with room_2) carrying the only door.
    ext = make_rect_walls("room_1", "r1")
    ext_no_n = [w for w in ext if w.id != "wall_r1_n"]
    shared = Wall(id="wall_shared", start=(10.0, 10.0), end=(0.0, 10.0), bounds_rooms=["room_1", "room_2"])
    room_1 = make_room("room_1", "bedroom", [w.id for w in ext_no_n] + [shared.id])
    room_2_walls = make_rect_walls("room_2", "r2", x0=50.0, y0=50.0, x1=60.0, y1=60.0)
    room_2 = make_room("room_2", "hallway", [w.id for w in room_2_walls], x0=50.0, y0=50.0, x1=60.0, y1=60.0)
    door = Opening(id="opening_door1", opening_type="door", wall_id="wall_shared", offset_ft=2.0, width_ft=3.0)
    layout = make_layout([room_1, room_2], ext_no_n + [shared] + room_2_walls, [door])

    failures = RULE.check(layout, CTX)
    assert len(failures) == 1
    assert failures[0]["entity_ids"] == ["room_1"]
    assert failures[0]["check"] == "irc_r310_1"


def test_non_bedroom_with_no_exterior_opening_passes():
    walls = make_rect_walls("room_1", "r1")
    room = make_room("room_1", "living", [w.id for w in walls])
    layout = make_layout([room], walls, [])
    assert RULE.check(layout, CTX) == []


def test_multiple_bedrooms_one_compliant_one_not():
    walls_1 = make_rect_walls("room_1", "r1", x0=0.0, y0=0.0, x1=10.0, y1=10.0)
    room_1 = make_room("room_1", "bedroom", [w.id for w in walls_1], x0=0.0, y0=0.0, x1=10.0, y1=10.0)
    window = Opening(id="opening_win1", opening_type="window", wall_id="wall_r1_n", offset_ft=2.0, width_ft=3.0)

    walls_2 = make_rect_walls("room_2", "r2", x0=50.0, y0=50.0, x1=60.0, y1=60.0)
    room_2 = make_room("room_2", "bedroom", [w.id for w in walls_2], x0=50.0, y0=50.0, x1=60.0, y1=60.0)

    layout = make_layout([room_1, room_2], walls_1 + walls_2, [window])
    failures = RULE.check(layout, CTX)
    assert len(failures) == 1
    assert failures[0]["entity_ids"] == ["room_2"]
