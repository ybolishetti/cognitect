"""
Shared DXF/JSON plan importer — parses an uploaded file into a FloorPlanState.

Used by both api/routes/load.py (v1, in-memory _PLANS dict) and
api/routes/plans_v2.py (v2, Supabase-backed /v2/plans/upload). The parsing
logic itself (unit detection, wall-layer heuristics, hatch/line-cycle/polyline
pass priority, outlier filtering, min/max sqft clamps) is unchanged from the
original v1-only implementation — only the plan_id/PlanManager/storage wiring
that used to live alongside it has been moved out to the two callers.
"""
...
def parse_json_to_state(raw: bytes) -> FloorPlanState:
    """Parse native Cognitect FloorPlanState JSON bytes into a FloorPlanState."""
    try:
        data = json.loads(raw)
        return FloorPlanState(**data)
...
def parse_dxf_to_state(raw: bytes, filename: str = "") -> FloorPlanState:
    """
    Parse a DXF file's bytes and extract rooms from closed polylines, LINE walls,
    or HATCH fills, into a FloorPlanState.
    ...
    logger.info(
        "Parsed DXF plan '%s' — %d rooms (source=%s, scale=%.6f, insunits=%d, raw=%d, filtered=%d)",
